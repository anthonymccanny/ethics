...
count_exclude 
#58 countries to be excluded from chi-squared test due to less than 5 observations 
#for one the bins 

chi_test_df <- ls_gallup_red_w %>%
  filter(!(COUNTRYNEW %in% count_exclude)) %>%
  group_by(COUNTRYNEW) %>%
  distinct(expected_val, .keep_all = TRUE) %>% #leaving us with 86 countries 
  summarise(n = sum(expected_val), 
            estimate = list(chisq.test(actual_val, p = expected_val/n)$statistic),
            p_value = list(chisq.test(actual_val, p = expected_val/n)$p.value),
            parameter = list(chisq.test(actual_val, p = expected_val/n)$parameter)) %>%
...
create_histograms <- function(data, filename) {
  histograms <- data %>%
    ...
    labs(x = "Weighted Life Satisfaction",
         y = "Frequency") +
    theme_minimal() +
...
output_filenames <- c("wvs_histograms_set1.pdf", "wvs_histograms_set2.pdf", "wvs_histograms_set3.pdf",
                      "wvs_histograms_set4.pdf", "wvs_histograms_set5.pdf", "wvs_histograms_set6.pdf",
                      "wvs_histograms_set7.pdf")
...
for (i in seq_along(country_chunks)) {
  start <- (i - 1) * 10 + 1
  end <- min(i * 10, length(unique_countries))
  selected_countries <- unique_countries[start:end]
  
  country_chunks[[i]] <- ls_wvs_red_w %>% filter(country_name %in% selected_countries)
}

# Create histograms for each group of 10 countries and save as PDFs
for (i in seq_along(country_chunks)) {
  create_histograms(country_chunks[[i]], output_filenames[i])
}
